    ...
    for (let i=0;i<this.notePos.length;i++){
      for (let j=0;j<this.notePos[i].length;j++){
        ...
        if (this.noteState[i][j]>0) {
          ...
          let c1 = color(255,100,0,alpha);
          let c2 = color(0,100,255,alpha);
          let mix = lerpColor(c1, c2, map(i, 0, this.notePos.length, 0, 1));
          fill(mix);

          let s = this.noteState[i][j];

          ellipse(x, y, this.noteSize * s * random(0.5), this.noteSize * s * random(0.5));
        }

        this.noteState[i][j] += 0.01;
        this.noteState[i][j]=constrain(this.noteState[i][j],-3,3);
      }
    }
  }
  /////////////////////////////////
